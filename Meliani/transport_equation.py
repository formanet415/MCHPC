"""
Implementation of a Euler scheme solver of the 1D advection equation.
Equation: 
∂_t u + a_0 ∂_x u = 0
"""
import numpy as np
...
L = 1.5e11 # m (1 au)
sigma = 0.1*L
nsigma = 0.1 # normalised
numax = 1 # normalised max speed
ttravel = L/a0
...
class InitialProfile:
    ...
    def set_gaussian_profile(self, grid, x0, sigma):
        ...
        if self.boundary_conditions == 'periodic':
            self.profile += self.Gaussian(grid + 1, x0, sigma) + self.Gaussian(grid - 1, x0, sigma) # simple periodic extension by one period


    def Gaussian(self, x, mu, sigma):
        return np.exp(-0.5*((x - mu)/sigma)**2) # max = 1 - normalised
    
    def get_physical_profile(self):
        raise NotImplementedError('Physical profile conversion not implemented yet.')
...
class TransportEquation1D:
    ...
    def run_simulation(self, tmax):
        dt = self.determine_time_step()
        nsteps = int(tmax / dt)
        u = self.profile.profile
        dx = 1 / self.Nx

        for n in range(nsteps):
            # update the points from 1 to end-1 (the first and last points are for boundary conditions)
            du = self.du(u, dx, dt)
            u[1:-1] += du
            ...
            if n%10 == 0:
                print(f'Step {n}/{nsteps}, Time {n*dt:.2f}s')
                print(self.profile.profile[0:10])
                self.profile.set_profile(u)
                self.profile.plot_profile()
...
if __name__ == "__main__":
    ...
    initial_profile.plot_profile()
    simulator = TransportEquation1D(a0, L, nx, Ccond, initial_profile)
    simulator.run_simulation(tmax=ttravel)